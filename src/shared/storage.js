...
// Issue #1: Expose Storage API to global scope for content script access
if (typeof window !== 'undefined') {
  // Create simple synchronous storage API wrapper
  const storageApi = {
    /**
     * 初始化儲存 API - 在 content script 中調用
     */
    init: async function() {
      try {
        // 驗證 chrome.storage 可用
        if (!chrome || !chrome.storage || !chrome.storage.local) {
          console.error('[storage.js] Chrome storage API not available');
          return false;
        }
        console.log('[storage.js] Storage API initialized');
        return true;
      } catch (error) {
        console.error('[storage.js] Error initializing storage:', error);
        return false;
      }
    },
    ...
    getItem: function(key) {
      try {
        // 使用簡單的同步方式從 localStorage 備份
        const value = localStorage.getItem(`_scm_${key}`);
        if (value !== null) {
          return value;
        }
        return null;
      } catch (error) {
        console.warn('[storage.js] Error getting item:', error);
        return null;
      }
    },
    ...
    setItem: function(key, value) {
      try {
        localStorage.setItem(`_scm_${key}`, value);
        // 同時保存到 chrome.storage.local
        if (chrome && chrome.storage && chrome.storage.local) {
          chrome.storage.local.set({ [key]: value }, () => {
            if (chrome.runtime.lastError) {
              console.warn('[storage.js] Error setting in chrome storage:', chrome.runtime.lastError);
            }
          });
        }
        return true;
      } catch (error) {
        console.warn('[storage.js] Error setting item:', error);
        return false;
      }
    },
    ...
    removeItem: function(key) {
      try {
        localStorage.removeItem(`_scm_${key}`);
        // 同時從 chrome.storage.local 移除
        if (chrome && chrome.storage && chrome.storage.local) {
          chrome.storage.local.remove(key, () => {
            if (chrome.runtime.lastError) {
              console.warn('[storage.js] Error removing from chrome storage:', chrome.runtime.lastError);
            }
          });
        }
        return true;
      } catch (error) {
        console.warn('[storage.js] Error removing item:', error);
        return false;
      }
    },
    ...
    clear: function() {
      try {
        ...
        keys.forEach(key => {
          if (key.startsWith('_scm_')) {
            localStorage.removeItem(key);
          }
        });
        // 同時清除 chrome.storage.local
        if (chrome && chrome.storage && chrome.storage.local) {
          chrome.storage.local.clear(() => {
            if (chrome.runtime.lastError) {
              console.warn('[storage.js] Error clearing chrome storage:', chrome.runtime.lastError);
            }
          });
        }
        return true;
      } catch (error) {
        console.warn('[storage.js] Error clearing storage:', error);
        return false;
      }
    },
    ...
    length: function() {
      try {
        return Object.keys(localStorage).filter(key => key.startsWith('_scm_')).length;
      } catch (error) {
        console.warn('[storage.js] Error getting length:', error);
        return 0;
      }
    },
    ...
    keys: function() {
      try {
        return Object.keys(localStorage)
          .filter(key => key.startsWith('_scm_'))
          .map(key => key.substring(5)); // 移除 _scm_ 前綴
      } catch (error) {
        console.warn('[storage.js] Error getting keys:', error);
        return [];
      }
    }
  };
  ...
  // 暴露到全局作用域
  window._scm_storage = storageApi;
  console.log('[storage.js] StorageAPI exposed as window._scm_storage');
}
